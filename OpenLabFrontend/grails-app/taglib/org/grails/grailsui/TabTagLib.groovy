...
class TabTagLib {
    ...
    static void clearCurrentTabViewId() {
        ...
        // We have to make sure that tabs are not retained in memory between requests.
        // Since we may have nested tab views, that means we have to track which is the
        // top level one and only clear the 'tabs' thread local once we have finished
        // processing the top level tab view.
        if (list.size() == 0) {
            tabs.get().clear()
        }
    }
